...
from .metrics import bbox_iou, probiou, wiou_v3_loss
...
class BboxLoss(nn.Module):
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask):
        ...
        loss_iou = wiou_v3_loss(pred_bboxes[fg_mask], target_bboxes[fg_mask]).mean()
...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk=10):  # model must be de-paralleled
        ...
        class_counts = [229, 91, 32, 31, 45, 75, 22]  # Update this to match your dataset
        self.cls_loss_fn = CBFocalLoss(gamma=2.0, class_counts=class_counts, device=device)
...
class CBFocalLoss(nn.Module):
    def __init__(self, gamma=2.0, alpha=0.25, class_counts=None, device=None):
        super(CBFocalLoss, self).__init__()
        self.gamma = gamma
        self.alpha = alpha

        if class_counts is None:
            raise ValueError("You must provide class_counts to use CBFocalLoss.")
    ...
    def forward(self, pred, label):
        """
        pred: [B, N, C] - logits
        label: [B, N, C] - one-hot encoded targets
        """
        pred_prob = pred.sigmoid()
        p_t = label * pred_prob + (1 - label) * (1 - pred_prob)
        modulating_factor = (1.0 - p_t) ** self.gamma
        alpha_factor = label * self.alpha + (1 - label) * (1 - self.alpha)
        # Broadcast class weights properly
        weight_factor = self.class_weights.view(1, 1, -1) * alpha_factor

        loss = F.binary_cross_entropy_with_logits(pred, label, reduction="none")
        loss *= modulating_factor * weight_factor
